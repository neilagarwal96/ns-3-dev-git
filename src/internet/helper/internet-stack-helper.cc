...
InternetStackHelper::Initialize ()
{
  ...
  SetFlonase ("ns3::FlonaseL4Protocol");
...
InternetStackHelper::InternetStackHelper (const InternetStackHelper &o)
{
  ...
  m_flonaseFactory = o.m_flonaseFactory;
...
void
InternetStackHelper::SetFlonase (const std::string tid)
{
  m_flonaseFactory.SetTypeId (tid);
}
...
void
InternetStackHelper::SetFlonase (std::string tid, std::string n0, const AttributeValue &v0)
{
  m_flonaseFactory.SetTypeId (tid);
  m_flonaseFactory.Set (n0,v0);
}
  ...
  if (m_ipv4Enabled || m_ipv6Enabled)
    {
      ...
      node->AggregateObject (m_flonaseFactory.Create<Object> ());
